...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
  ...
  int wid = grid[0].size();
  int area = hei * wid;
  float cellbelief = 1.0 / area;

  for (int i = 0; i<hei; i++) {
    row1.clear();
    for (int j = 0; j<wid; j++) {
        row1.push_back(cellbelief);
    }
    newGrid.push_back(row1);
  }
	return newGrid;
}
  ...
  float blurring) 
{
  // your code here
  vector < vector <float> > newGrid;
  float hei = beliefs.size();
  float wid = beliefs[0].size();
  float ione;
  float jone;
  for (int i = 0; i < beliefs.size(); i++) {
    for (int j = 0 ; j < beliefs.size();j++) {
        ione = (i + dy) % hei;
        jone = (j + dx) % wid;
        newGrid[int(ione)][int(jone)] = beliefs[i][j];
        }
    }
